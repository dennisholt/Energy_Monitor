...
Output measures:
  ...
  influx -precision 'rfc3339' -database 'battery_db' 
drop continuous query cq_inv_s30 on battery_db
influx -precision 'rfc3339' -database 'battery_db' -execute 'select * from m5.m5_energy' -format csv > m5_energy_Jan30.csv

select time, Ahr2empty, avgVoltage, local_dt, medianV, pakMinV, pakMinV_ID, pak_27, soc from d1.d1_lowPack where time > now() - 6h
