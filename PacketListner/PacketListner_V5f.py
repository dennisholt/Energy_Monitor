...
def main():
    ...
    node_list = []
    pack = Pack()
    while time.time() < start + 5: # run for 5 minutes
        #print('C while time < 10sec')
        raw_data, addr = packet_conn.recvfrom(1024) # get a packet
        ...
        # TCP
            ...
            src_port, dest_port, sequence, acknowledgement, flag_urg, flag_ack, flag_psh,\
        ...
        # UDP
            ...
            if dest_port == 18542:
                ...
                print('mesType= {}'.format(hex(mesType)))
            else: continue
            # collect list pack volt & temp
            # replace list items for packs with lowest so far
            # sort list and save lowest two (voltage & temp)
            if mesType == 0x415A and (pack.need_1_16 or pack.need_17_28):
                ...
                for i in range(0, records):
                    st = i * 11 + 12
                    node_id, min_v, max_v, min_t = struct.unpack('< B x H H B', data[st:st+7])
                    min_v = min_v / 1000
                    max_v = max_v / 1000
                    node_t = min_t - 40
