...
time.sleep(60)   #  let things get settled down
...
while True:
    today = date.today()
    fb = open_next_file("/home/pi/jeff/outfile" + str(today) + ".bin")
    day_start = time.time()
    ...
    #            for j in range(1,40):
                ...
                out = array('c')
                out.fromstring(tb)
                out.fromstring(chr(len(response)))
                out.extend(response)
                out.fromstring(chr(255)+chr(255))
                fb.write (out)
