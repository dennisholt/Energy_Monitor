   ...
   while need_packet:
      ...
      # UDP
         ...
         if dest_port == 18542:
            ...
            if mesType == 0x3233:
               soc, cap2empty, kWh_charge, kWh_discharge = struct.unpack('< 32x H 3f', data[:46])
               soc = soc / 100  
            ...
            #   out_c = str(soc) +', ' + str(shunt_v) + ', ' + str(shunt_c) + ', ' + str(cap2empty)
               print('SOC= {} Ahr to empty= {} kWh Charge= {} kWh Discharge= {}'.format(soc, cap2empty, kWh_charge, kWh_discharge))
            #    print(out)
               need_packet = False
         else: continue    
